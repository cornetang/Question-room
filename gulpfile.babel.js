...
function notiOpts(msg, onLast) {
	const opts = { title: "Qweet", message: msg, sound: true };
...
gulp.task("script", [ "clean:script" ], () => {
	return gulp.src("src/js/index.jsx")
		...
		.pipe(connect.reload())
		.pipe(notify(notiOpts("All scripts are built!", true)));
});
...
gulp.task("html", [ "clean:html" ], () => {
	...
	const favicon2 = gulp.src("./src/favicon/**/*.{svg,png,xml,json}")
...
gulp.task("watch", [ "connect" ], (callback) => {
